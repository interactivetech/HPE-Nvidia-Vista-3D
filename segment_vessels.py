import os
import requests
import json
from pathlib import Path
from tqdm import tqdm
import nibabel as nib
from dotenv import load_dotenv

# Load environment variables
load_dotenv()

# Configuration
IMAGE_SERVER_URL = os.getenv('IMAGE_SERVER', 'https://localhost:8888')
VISTA3D_INFERENCE_URL = "http://localhost:8000/v1/vista3d/inference"
PROJECT_ROOT = Path(os.getenv('PROJECT_ROOT', '.'))
NIFTI_INPUT_BASE_DIR = PROJECT_ROOT / "outputs" / "nifti"
SEGMENTATION_OUTPUT_BASE_DIR = PROJECT_ROOT / "outputs" / "segmentation"
VESSELS_OF_INTEREST = [v.strip().lower() for v in os.getenv('VESSELS_OF_INTEREST', '').split(',') if v.strip()]

def download_file(url: str, destination_path: Path):
    """Downloads a file from a URL with a progress bar."""
    print(f"Downloading {url} to {destination_path}...")
    try:
        response = requests.get(url, stream=True, verify=False) # verify=False for self-signed certs
        response.raise_for_status()
        total_size = int(response.headers.get('content-length', 0))
        block_size = 1024 # 1 Kibibyte
        
        destination_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(destination_path, 'wb') as f:
            with tqdm(total=total_size, unit='iB', unit_scale=True, desc=destination_path.name) as pbar:
                for chunk in response.iter_content(chunk_size=block_size):
                    if chunk:
                        f.write(chunk)
                        pbar.update(len(chunk))
        print(f"Successfully downloaded {destination_path}")
        return True
    except requests.exceptions.RequestException as e:
        print(f"Error downloading {url}: {e}")
        return False

def get_all_nifti_files(base_dir: Path):
    """Scans a base directory for all NIfTI files and returns their absolute paths."""
    nifti_files = []
    if not base_dir.exists():
        print(f"Warning: Input NIfTI directory not found: {base_dir}")
        return []

    for root, _, files in os.walk(base_dir):
        for file in files:
            if file.endswith(('.nii', '.nii.gz')):
                nifti_files.append(Path(root) / file)
    return nifti_files

def main():
    print("--- Vista3D Batch Segmentation Script ---")
    print(f"Input NIfTI directory: {NIFTI_INPUT_BASE_DIR.absolute()}")
    print(f"Output Segmentation directory: {SEGMENTATION_OUTPUT_BASE_DIR.absolute()}")
    if VESSELS_OF_INTEREST:
        print(f"Vessels of Interest (for potential filtering/highlighting): {', '.join(VESSELS_OF_INTEREST)}")
    print("------------------------------------------")

    # Create output directory
    SEGMENTATION_OUTPUT_BASE_DIR.mkdir(parents=True, exist_ok=True)

    all_nifti_files = get_all_nifti_files(NIFTI_INPUT_BASE_DIR)
    if not all_nifti_files:
        print("No NIfTI files found in the input directory. Exiting.")
        return

    print(f"Found {len(all_nifti_files)} NIfTI files to process.")
    print("\n--- Starting Segmentation ---")

    successful_segmentations = 0
    failed_segmentations = 0

    for nifti_file_path in tqdm(all_nifti_files, desc="Processing NIfTI files", unit="file"):
        try:
            # Determine relative path from input base dir
            relative_path = nifti_file_path.relative_to(NIFTI_INPUT_BASE_DIR)
            
            # Construct output path, preserving directory structure
            output_segmentation_path = SEGMENTATION_OUTPUT_BASE_DIR / relative_path.parent / f"{nifti_file_path.stem}_seg.nii.gz"
            output_segmentation_path.parent.mkdir(parents=True, exist_ok=True)

            # Construct URL for Vista3D (using host.docker.internal for container access)
            # The image server serves from PROJECT_ROOT, so relative_path is correct for URL
            vista3d_input_url = f"{IMAGE_SERVER_URL}/{relative_path}"

            payload = {"image": vista3d_input_url}
            if VESSELS_OF_INTEREST:
                payload["prompts"] = {"classes": VESSELS_OF_INTEREST}
            else:
                payload["prompts"] = {}
            headers = {"Content-Type": "application/json"}

            print(f"\n  Processing: {nifti_file_path.name}")
            print(f"    Input URL for Vista3D: {vista3d_input_url}")

            inference_response = requests.post(VISTA3D_INFERENCE_URL, json=payload, headers=headers, verify=False)
            inference_response.raise_for_status()
            result = inference_response.json()
            
            segmented_nifti_url = result.get("result")
            if not segmented_nifti_url:
                print(f"    Error: 'result' URL not found in inference response for {nifti_file_path.name}.")
                print(f"    Response: {result}")
                failed_segmentations += 1
                continue
            
            print(f"    Inference successful. Downloading segmented NIfTI from: {segmented_nifti_url}")

            if not download_file(segmented_nifti_url, output_segmentation_path):
                print(f"    Failed to download segmented NIfTI for {nifti_file_path.name}.")
                failed_segmentations += 1
                continue
            
            print(f"    Segmented file saved to: {output_segmentation_path.name}")
            successful_segmentations += 1

        except requests.exceptions.RequestException as e:
            print(f"\n  Error during inference request for {nifti_file_path.name}: {e}")
            print(f"  Please ensure the Vista3D Docker container is running and accessible at {VISTA3D_INFERENCE_URL}")
            failed_segmentations += 1
        except json.JSONDecodeError:
            print(f"\n  Error: Could not decode JSON response from inference server for {nifti_file_path.name}.")
            print(f"  Response content: {inference_response.text}")
            failed_segmentations += 1
        except Exception as e:
            print(f"\n  An unexpected error occurred for {nifti_file_path.name}: {e}")
            failed_segmentations += 1

    print("\n--- Segmentation Process Complete ---")
    print(f"Successful segmentations: {successful_segmentations}")
    print(f"Failed segmentations: {failed_segmentations}")
    print("-------------------------------------")
    print("You can now view the segmented files in your Streamlit app by selecting them from the 'outputs/segmentation' folder.")

if __name__ == "__main__":
    main()
