...
def export_multi_material_obj(meshes: list, obj_path: str, mtl_path: str, label_info: dict):
    """Export multiple meshes as a single OBJ file with material groups and MTL file."""
    # Create MTL file content
    mtl_content = []
    
    for i, mesh in enumerate(meshes):
        label_id = mesh.metadata.get('label_id', i)
        label_name = mesh.metadata.get('label_name', f'label_{i}')
        
        # Get color from label info
        info = label_info.get(int(label_id))
        if info:
            # Normalize RGB from 0-255 to 0-1
            r, g, b = [c / 255.0 for c in info['color'][:3]]
        else:
            # Default gray
            r, g, b = 0.5, 0.5, 0.5
        
        # Create material definition
        mat_name = f"mat_{label_name.replace(' ', '_')}"
        mtl_content.append(f"newmtl {mat_name}")
        mtl_content.append(f"Ka {r} {g} {b}")  # Ambient color
        mtl_content.append(f"Kd {r} {g} {b}")  # Diffuse color
        mtl_content.append(f"Ks 0.3 0.3 0.3")  # Specular color
        mtl_content.append(f"Ns 10.0")  # Specular exponent
        mtl_content.append("")
    ...
    obj_content = []
    obj_content.append(f"# Multi-material OBJ file")
    obj_content.append(f"# Generated by voxel2obj.py")
    obj_content.append(f"mtllib {os.path.basename(mtl_path)}")
    obj_content.append("")
    ...
    for i, mesh in enumerate(meshes):
        ...
        obj_content.append(f"# Object: {label_name}")
        obj_content.append(f"g {label_name.replace(' ', '_')}")
        ...
        # Write faces (adjust indices by vertex offset)
        for face in mesh.faces:
            # OBJ uses 1-based indexing
            f1 = face[0] + vertex_offset + 1
            f2 = face[1] + vertex_offset + 1
            f3 = face[2] + vertex_offset + 1
            obj_content.append(f"f {f1} {f2} {f3}")
    ...
    for fpath in nifti_files:
        ...
        try:
            ...
            if base_name == 'all' and label_info:
                ...
                # Export combined mesh as all.obj with material groups
                if meshes:
                    all_out_path = os.path.join(output_dir, "all.obj")
                    all_mtl_path = os.path.join(output_dir, "all.mtl")
                    export_multi_material_obj(meshes, all_out_path, all_mtl_path, label_info)
                    if verbose:
                        print(f"    - Exported combined mesh: {all_out_path}")
                        print(f"    - Exported materials: {all_mtl_path}")
