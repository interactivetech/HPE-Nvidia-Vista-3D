...
def get_patients_with_voxel_files() -> List[str]:
    """Get list of patient folders that have voxel files in their voxels folder."""
    try:
        # Load environment variables to get OUTPUT_FOLDER path
        from dotenv import load_dotenv
        load_dotenv()
        ...
        for entry in os.scandir(output_path):
            if entry.is_dir() and entry.name != 'uploads':
                ...
                if voxels_dir.exists() and voxels_dir.is_dir():
                    has_voxel_files = False
                    try:
                        for scan_dir in os.scandir(voxels_dir):
                            if scan_dir.is_dir():
                                # Check if this scan directory has nii.gz files
        ...
        st.error(f"Error getting patients with voxel files: {str(e)}")
        return []


def get_voxel_scans_for_patient(patient_id: str) -> List[str]:
    """Get list of scan directories that contain voxel files for a specific patient."""
    try:
        # Load environment variables to get OUTPUT_FOLDER path
        from dotenv import load_dotenv
        load_dotenv()
        ...
        # Get list of scan directories that contain nii.gz files
        scan_dirs = []
        try:
            for scan_dir in os.scandir(voxels_dir):
                if scan_dir.is_dir():
...
def render_nifti2obj_tools():
    """Render Voxel to OBJ conversion tools."""
    st.subheader("🫁 Voxel to OBJ Conversion")
    st.markdown("""
    Convert voxel NIfTI (.nii.gz) files to 3D OBJ mesh format for visualization and 3D printing.
    This tool processes files from the voxels/ directory and creates high-quality surface meshes with proper colors and materials.
    """)
    ...
    with col1:
        ...
        # Patient selection - only show patients with voxel files
        patient_folders = get_patients_with_voxel_files()
        
        if not patient_folders:
            st.warning("⚠️ No patient folders with voxel files found. Please check your OUTPUT_FOLDER path in .env file and ensure patients have been processed through Vista3D segmentation.")
            return
        
        # Get previously selected patients to detect changes
        prev_selected_patients = st.session_state.get("nifti2obj_patients", [])
        
        selected_patients = st.multiselect(
            "Select Patients",
            options=patient_folders,
            default=patient_folders[:1] if patient_folders else [],  # Select first patient by default
            help="Select one or more patient folders to process. Leave empty or uncheck all to process no patients.",
            key="nifti2obj_patients"
        )
        ...
        if selected_patients != prev_selected_patients:
            ...
            if "nifti2obj_scans" in st.session_state:
                # Get current available scans for selected patients
                current_available_scans = []
                for patient in selected_patients:
                    scans = get_voxel_scans_for_patient(patient)
                ...
                valid_scans = [scan for scan in st.session_state["nifti2obj_scans"] if scan in current_available_scans]
                st.session_state["nifti2obj_scans"] = valid_scans
        ...
        if selected_patients:
            ...
            if unique_scans:
                ...
                prev_selected_scans = st.session_state.get("nifti2obj_scans", [])
                ...
                # If no valid previous selections, default to first scan
                default_scans = valid_prev_scans if valid_prev_scans else (unique_scans[:1] if unique_scans else [])
                
                selected_scans = st.multiselect(
                    "Select Scans to Process",
                    options=unique_scans,
                    default=default_scans,
                    help="Select specific voxel scans to convert to OBJ format. Leave empty or uncheck all to process no scans.",
                    key="nifti2obj_scans"
                )
            else:
                ...
                if "nifti2obj_scans" in st.session_state:
                    st.session_state["nifti2obj_scans"] = []
        else:
        ...
        with st.expander("⚙️ Advanced Conversion Options"):
            ...
            with col_a:
                threshold = st.slider(
                    ...
                    value=0.1, 
                    step=0.01,
                    help="Threshold for generating the iso-surface (applies to single-label files)"
                )
                ...
                smoothing = st.slider(
                    ...
                    value=10, 
                    step=1,
                    help="Number of smoothing iterations to apply to the mesh"
                )
            ...
            with col_b:
                decimation = st.slider(
                    ...
                    value=0.5, 
                    step=0.05,
                    help="Mesh decimation fraction (1.0 = no decimation, lower = more simplified mesh)"
                )
                
                close_boundaries = st.checkbox(
                    "Close Mesh Boundaries", 
                    value=True, 
                    help="Attempt to close boundary holes to make meshes watertight"
                )
            
            hole_filling_method = st.selectbox(
                "Hole Filling Method",
                options=["convex", "planar"],
                index=0,
                help="Method for filling holes: 'convex' uses convex hull, 'planar' uses simple triangulation"
            )
    ...
    with col2:
        ...
        conversion_clicked = st.button("🫁 Start Voxel to OBJ Conversion", key="start_nifti2obj", type="primary", disabled=button_disabled)
    
    # Check if conversion button was clicked
    if conversion_clicked:
        with st.spinner("Starting Voxel to OBJ conversion..."):
            ...
            # Process each patient-scan combination
            all_output_lines = []
            total_combinations = len(selected_patients) * len(selected_scans)
            ...
            for patient in selected_patients:
                for scan in selected_scans:
                    ...
                    base_progress = int((current_combination - 1) / total_combinations * 100)
                    
                    try:
                        # Prepare command arguments
                        cmd_args = [
                            "python", "utils/voxel2obj.py",
                            "--patient", patient,
                            "--scan", scan,
                            "--threshold", str(threshold),
                            "--smoothing", str(smoothing),
                            "--decimation", str(decimation),
                            "--hole-filling", hole_filling_method,
                            "-v"  # Verbose output
                        ]
                        
                        if not close_boundaries:
                            cmd_args.append("--no-close-boundaries")
                        
                        # Update status
                        status_text.text(f"🫁 Converting {patient}/{scan}... ({current_combination}/{total_combinations})")
                        progress_bar.progress(base_progress + 5)
                        ...
                        while True:
                            ...
                            if output:
                                ...
                                all_output_lines.append(f"[{patient}/{scan}] {output.strip()}")
                                
                                # Update output display (keep last 20 lines)
                                recent_output = all_output_lines[-20:]
                                output_placeholder.text_area(
                                    "Conversion Output:", 
                                    value="\n".join(recent_output), 
                                    height=300,
                                    disabled=True,
                                    key=f"nifti2obj_output_realtime_{len(all_output_lines)}"
                                )
                                
                                # Update progress within this scan
                                scan_progress = base_progress + int(15 / total_combinations)
                                if "Converting" in output or "Processing" in output:
                                    scan_progress = base_progress + int(30 / total_combinations)
                                elif "Writing OBJ" in output or "Exported" in output:
                                    scan_progress = base_progress + int(60 / total_combinations)
                                elif "Done" in output:
                                    scan_progress = base_progress + int(90 / total_combinations)
                                
                                progress_bar.progress(min(scan_progress, 99))
                        ...
                        if return_code != 0:
                            ...
                            st.warning(f"⚠️ Conversion failed for {patient}/{scan}")
                        
                    except Exception as e:
                        overall_success = False
                        error_msg = f"Error processing {patient}/{scan}: {str(e)}"
                        all_output_lines.append(f"[ERROR] {error_msg}")
                        st.error(error_msg)
            
            # Final status update
            if overall_success:
                progress_bar.progress(100)
                status_text.text("✅ All conversions completed successfully!")
                st.success("Voxel to OBJ conversion completed successfully!")
                st.balloons()
            else:
                progress_bar.progress(100)
                status_text.text("⚠️ Some conversions had errors")
                st.warning("Some conversions completed with errors. Check output for details.")
            
            # Show final output
            final_output = "\n".join(all_output_lines)
            st.text_area("Final Output:", final_output, height=400, disabled=True, key="nifti2obj_output_final")
...
def main():
    ...
    # Voxel to OBJ Conversion Section
    render_nifti2obj_tools()
