...
def generate_patient_cards_data(analysis_data: List[Dict]) -> Dict:
    """Generate patient cards data for display in app.py."""
    ...
    # Prepare patient cards data
    patient_cards = []
    
    for patient in analysis_data:
        if patient.get('error'):
            # Create error card
            card_data = {
                'patient_id': patient['patient_id'],
                'status': 'error',
                'error_message': patient['error'],
                'ct_scans': 0,
                'voxel_files': 0,
                'mesh_files': 0,
                'ply_files': 0,
            ...
            if patient.get('ct_scans'):
                for ct_scan_info in patient['ct_scans']:
                    ...
                    ct_scan_details.append({
                        'name': scan_display_name,
                        'original_name': ct_scan_name,
                        'size_bytes': ct_scan_size,
                        'size_display': format_file_size(ct_scan_size),
                        'has_voxel_file': has_voxel_file,
                        'voxel_count': voxel_count,
                        'mesh_count': mesh_count,
                        'ply_count': ply_count,
                        'has_voxel_data': has_voxel_file or voxel_count > 0,
                        'has_mesh_data': mesh_count > 0,
                        'has_ply_data': ply_count > 0
                    })
            ...
            card_data = {
                ...
                'status': 'success',
                'error_message': None,
                'ct_scans': patient.get('total_ct_scans', 0),
                'voxel_files': patient.get('total_voxel_files', 0),
                'mesh_files': patient.get('total_mesh_files', 0),
                'ply_files': patient.get('total_ply_files', 0),
                'total_size': format_file_size(patient.get('total_patient_size_bytes', 0)),
                'total_size_bytes': patient.get('total_patient_size_bytes', 0),
                'ct_scan_details': ct_scan_details,
                'has_voxel_data': patient.get('total_voxel_files', 0) > 0,
                'has_mesh_data': patient.get('total_mesh_files', 0) > 0,
                'has_ply_data': patient.get('total_ply_files', 0) > 0,
                'structure_type': patient.get('structure_type', 'current')
            }
        
        patient_cards.append(card_data)
    
    # Summary statistics
    summary_stats = {
        'total_patients': total_patients,
        'patients_with_ct_scans': patients_with_data,
        'patients_with_voxels': patients_with_voxels,
        'patients_with_meshes': patients_with_meshes,
        'patients_with_ply': patients_with_ply,
        'total_ct_scans': total_ct_scans,
        'total_ct_size': format_file_size(total_ct_size_bytes),
        'total_voxel_size': format_file_size(total_voxel_size_bytes),
        'total_mesh_size': format_file_size(total_mesh_size_bytes),
        'total_ply_size': format_file_size(total_ply_size_bytes),
        'total_data_size': format_file_size(total_data_size_bytes),
...
def get_patient_cards_data(image_server_url: str = None) -> Dict:
    """Get patient cards data for display in app.py."""
    ...
    # Generate patient cards data
    return generate_patient_cards_data(analysis_data)
...
def main():
    ...
    parser.add_argument("--cards", action="store_true", help="Output patient cards data as JSON")
    ...
    # Generate and display report or cards data
    if args.cards:
        # Output patient cards data as JSON
        cards_data = generate_patient_cards_data(analysis_data)
